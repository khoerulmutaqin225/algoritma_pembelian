...
class ReportExcelAlgoritmaPembelianController(http.Controller):
    ...
    def get_algoritma_pembelian_excel_report(self, data=None, **args):
        ...
        atas_style = workbook.add_format(
            {'font_name': 'Times', 'bold': True, 'align': 'left'})
        atas_isi_style = workbook.add_format(
            {'font_name': 'Times', 'bold': False, 'align': 'left'})
        header_style = workbook.add_format(
            {'font_name': 'Times', 'bold': True, 'left': 1, 'bottom': 1, 'right': 1, 'top': 1, 'align': 'center'})
        text_style = workbook.add_format(
            {'font_name': 'Times', 'bold': False, 'left': 1, 'bottom': 1, 'right': 1, 'top': 1, 'align': 'left'})
        ...
        for atas in data:
            ...
            record_line = request.env['algoritma.pembelian.line'].search(
                [('algoritma_pembelian_id', '=', atas.id)])
...
# class AlgoritmaPembelianRestApi(http.Controller): 
    ...
    #     brands_obj = request.env['algoritma.brand'].sudo().search([(
    #         'name', 'in', name_brand)])
